1#packages required in the code 
# clear the workspace
rm(list = ls())
...
getcsv <- memoise(function(keyword){
  ...
  write.csv(df, file = "C:/Users/Mrinal Malik/Desktop/Final report Sentiment/demo.csv")
})
...
getTermMatrix <- memoise(function(keyword) {
  #read csv file  
  extract_tw=read.csv("C:/Users/Mrinal Malik/Desktop/Final report Sentiment/demo.csv")
...
getTermMatrixSentiment <- memoise(function() {
  #reading the csv 
  tw=read.csv("C:/Users/Mrinal Malik/Desktop/Final report Sentiment/demo.csv")
  tweeter = tw$text
...
ui2 <- function(){tagList(shinyUI(fluidPage(
  ...
  navbarPage("Social Media Analytics App",theme = shinythemes::shinytheme("superhero"),
...
server = (function(input, output,session) {
  ...
  output$picture <-
    renderText({
      c(
        '<img src="',
          "https://www.wordclouds.com/gallery/business.png",
  ...
  database<- read.csv("C:/Users/Mrinal Malik/Desktop/Final report Sentiment/database.csv")
  observe({ 
